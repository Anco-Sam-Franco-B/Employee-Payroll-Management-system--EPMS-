...
export const viewSalaries = async (req, res) => {
  try {
    ...
    db.query(sql, (err, data) => {
      if (err) return res.status(500).json({ message: "Error fetching salaries", error: err.message });
...
export const processPayroll = async (req, res) => {
  ...
  try {
    ...
    db.query(sql, [depId, empId, netSalary, month], (err) => {
      if (err) return res.status(500).json({ message: "Error processing payroll", error: err.message });
